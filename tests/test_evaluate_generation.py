...
class TestAnalyzeResponse:
    ...
    def test_detects_non_empty_response(self):
    ...
    def test_detects_no_citation(self):
    ...
    def test_detects_hedging(self):
        ...
        assert any("possivelmente" in w for w in r.hedging_words)
...
class TestGenerationEvaluator:
    def test_loads_only_retrieval_queries(self, evaluator):
        ...
        ids = [q[0] for q in evaluator.queries]
    ...
    @patch("evaluation.evaluate_generation.VectorSearch")
    def test_evaluate_basic(self, MockSearch, MockLlm, evaluator):
        mock_search = MockSearch.return_value
        mock_search.search.return_value = [
            {"regulation_id": "DOC-1", "text": "Artigo sobre voo.", "score": 0.9}
        ]

        mock_llm = MockLlm.return_value
        mock_llm.generate_with_context.return_value = (
            "Conforme ICA-96-1-art10, o artigo estabelece as regras de voo."
        )

        result = evaluator.evaluate(k=5, sample=None, workers=1)
    ...
    @patch("evaluation.evaluate_generation.LlamaModel")
    @patch("evaluation.evaluate_generation.VectorSearch")
    def test_evaluate_empty_results(self, MockSearch, MockLlm, evaluator):
        mock_search = MockSearch.return_value
        mock_search.search.return_value = []

        result = evaluator.evaluate(k=5, workers=1)
    ...
    @patch("evaluation.evaluate_generation.LlamaModel")
    @patch("evaluation.evaluate_generation.VectorSearch")
    def test_evaluate_sample(self, MockSearch, MockLlm, evaluator):
        mock_search = MockSearch.return_value
        mock_search.search.return_value = []

        result = evaluator.evaluate(k=1, sample=1, workers=1)
        assert result.total_queries == 1

    @patch("evaluation.evaluate_generation.LlamaModel")
    @patch("evaluation.evaluate_generation.VectorSearch")
    def test_save_results(self, MockSearch, MockLlm, evaluator, tmp_path):
        mock_search = MockSearch.return_value
        mock_search.search.return_value = []
...
class TestPrintReport:
    @patch("evaluation.evaluate_generation.LlamaModel")
    @patch("evaluation.evaluate_generation.VectorSearch")
    def test_print_report_runs(self, MockSearch, MockLlm, evaluator, capsys):
        mock_search = MockSearch.return_value
        mock_search.search.return_value = [
            {"regulation_id": "D1", "text": "t", "score": 0.9}
        ]
        mock_llm = MockLlm.return_value
        mock_llm.generate_with_context.return_value = "Possivelmente algo."
